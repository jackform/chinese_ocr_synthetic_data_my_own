...
def getAllCharactersFromFile(characters_file_path):
    characters_set = set()
    with open(characters_file_path) as f:
        data = f.readlines()
...
def get_contents(id_character_dict, length_range_tuple, line_number=1):
    ...
    for i in range(line_number):
        # 每行随机characters_length_tuple[1]到characters_length_tuple[2]个文字(这里是3~6个)
...
def saveImage2Dir(image, image_save_dir, image_name='test_image'):
    if type(image) == list:
        ...
        # image.resize((32, scale_height), Image.ANIALIAS)
        # image = image.resize((32, scale_height))
        # print("saved width and height")
        # print(image.width)
        # print(image.height)
...
def mergeBgimgAndTxtimgPoints(left_center, points):
    ...
    # 取出文本在图片中的顶点
    top = int(center_line - max(points[:, 1]) - min(points[:, 1]) / 2)

    # 将单独的文字框架的坐标转换成在图片中的坐标
    for k in range(0, 4):
...
def addRandomInROI(roi):
    ...
    max_randint = int((bottom - top) / 6)
    new_left = random.randint(left - max_randint, int(left + max_randint / 2))
    new_top = random.randint(top - max_randint, int(top + max_randint / 2))
    new_right = random.randint(int(right - max_randint / 2), right + max_randint)
