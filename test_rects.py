...
# python pse
from dataloader.rects_test_loader import ReCTSTestDataloader
...
def test(args):
    data_loader = ReCTSTestDataloader(long_size=args.long_size)
    ...
    if args.arch == "resnet50":
        ...
        model = PSENet(backbone="mobilenetv2", pretrained=False, result_num=7, scale=args.scale)
    ...
    for idx, (org_img, img) in enumerate(test_loader):
        ...
        # imshow(torchvision.utils.make_grid(text))
        text = text.data.cpu().numpy()[0].astype(np.uint8)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default="checkpoints/ReCTS_mobilenetV2_bs_16_ep_27/checkpoint.pth.tar",
