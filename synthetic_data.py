...
def main():
    ...
    root_dir = '../ocr_data/generation'
    data = OCRData(root_dir)
    data.makeNeededDir()
    # data.args['classes_number'] = 500
    data.args['classes_number'] = 5990
    print('The arguments :')
    print(data.args)

    data.saveTopNCharacters2File(data.args['characters_file_path'], data.args['classes_number'],
                                 data.args['id_character_file_path'])
    data.args['image_number'] = data.args['classes_number'] * 80
...
class OCRData(object):
    ...
    def setArguments(self):
        ...
        # 背影图路径
        self.args['background_image_dir'] = '../Chinese_OCR_synthetic_data-master/background/'
        # 字体路径
        self.args['fonts_dir'] = '../Chinese_OCR_synthetic_data-master/fonts'
        # 所有需要生成的文字数据
        # self.args['characters_file_path'] = 'data/train_ocr/characters.txt'
        self.args['characters_file_path'] = 'data/train_ocr/char_std_5990.txt'
        self.args['classes_number'] = 5
        self.args['id_character_file_path'] = os.path.basename(self.args['characters_file_path']).split('.')[
                                                  0] + '_top_%d.txt' % self.args['classes_number']
    ...
    def synthesizeAllImages(self, image_number):
        ...
        for i in tqdm.tqdm(range(start_index, image_number)):
            ...
            # 对bg_img_list和font_list两个list执行utils.getRandomOneFromList，随机挑选两个list的中一个元素
    ...
    def saveAnnotation(self, content_index, points, image_index):
        for index, one_content in enumerate(content_index):
            ...
            with codecs.open(ann_path, 'w', encoding='utf-8') as file:
                ...
                words = [self.id_character_dict[i] for i in one_content]
                words = ''.join(words)
                file.write(words)
    ...
    def putContent2Image(self, mulcontents, background_image_path, font_path, add_rectangle=0, resize_rate=2):
        try:
            ...
            while font_size_max < self.args['font_size_min']:
                ...
                font_size_max = image.size[0] / self.args['characters_length_tuple'][1]
            font_size = random.randint(int(self.args['font_size_min']), int(font_size_max))
            ...
            left_center_point = (random.randint(0, image.size[0] - font_size * max([len(i) for i in mulcontents])),
                                 random.randint(font_size * len(mulcontents),
                                                image.size[1] - int(font_size * len(mulcontents) / 2)))
            ...
            for content in mulcontents:
                ...
                for character in content:
                    image, points = self.putOneCharacter2Image(character, image, font_path, font_size,
    ...
    def restoreFromPartImageDir(self):
        ...
        if max_index != 0:
            print(
                'There are %d images had been generated before. Do you want to continue from that? "y" will continue or "n" will recover.' % max_index)
            choose = raw_input()
